...
describe("DownloadVideoHandler", () => {
  it("returns success on completed download, keeps a specific status untouched", async () => {
    const dl = { download: vi.fn(async () => ({ id: 1 })) };
    const videoRepo = {
      byId: vi.fn(() => ({ id: 1, availabilityStatus: "available" })),
    ...
    expect(r.success).toBe(true);
    expect(videoRepo.setAvailability).not.toHaveBeenCalled();
  });

  // Flat-playlist sync often stores items as "unknown"; a completed download
  // is the availability check, so the status must reflect it afterwards.
  it("lifts unknown availability to available after a successful download", async () => {
    const dl = { download: vi.fn(async () => ({ id: 1 })) };
    const videoRepo = {
      byId: vi.fn(() => ({ id: 1, availabilityStatus: "unknown" })),
      setAvailability: vi.fn(),
    };
    const h = new DownloadVideoHandler(
      dl as unknown as ConstructorParameters<typeof DownloadVideoHandler>[0],
    ...
    const r = await h.handle(fakeJob({ videoId: 1, kind: "audio" }, "download_video"));
    expect(r.success).toBe(true);
    expect(videoRepo.setAvailability).toHaveBeenCalledWith(1, "available", null);
  });
